<template>
  <section id="catalogo" class="py-12 md:py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
      ...
      <div class="inline-flex p-1.5 bg-surface-warm rounded-2xl border border-border-tab shadow-inner self-start md:self-auto overflow-x-auto max-w-full">
        <button
          type="button"
          ...
          :class="[
            ...
            activeCategory === 'all'
              ? 'bg-surface text-primary-dark shadow-sm font-semibold'
              : 'text-muted hover:text-main'
          ]"
        >
          <IconBase name="sparkles" fontSize="1rem" />
          <span>Todos</span>
          <span class="text-xs px-2 py-0.5 rounded-full bg-primary-soft font-bold text-primary-dark">
            {{ products.length }}
          </span>
        </button>
      ...
      <div class="relative w-full md:w-72">
        <input
          ...
          class="w-full pl-10 pr-4 py-2.5 bg-surface border border-border-warm rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all shadow-xs"
        />
        <svg
          class="w-4 h-4 text-light absolute left-3.5 top-1/2 -translate-y-1/2"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-for="i in 6" :key="i" class="animate-pulse bg-surface rounded-3xl p-4 border border-border-subtle">
        <div class="h-64 bg-surface-warm rounded-2xl mb-4"></div>
        <div class="h-6 bg-surface-warm rounded w-3/4 mb-2"></div>
        <div class="h-4 bg-surface-warm rounded w-full mb-2"></div>
        <div class="h-4 bg-surface-warm rounded w-2/3 mb-4"></div>
        <div class="h-10 bg-surface-warm rounded-xl"></div>
      </div>
    </div>

    <!-- Empty Results State -->
    <div v-else-if="filteredProducts.length === 0" class="text-center py-16 bg-surface/70 rounded-3xl border border-border-subtle p-8 max-w-lg mx-auto">
      <div class="mb-3">
        <IconBase name="cupcake" fontSize="3.5rem" />
      </div>
      <h3 class="text-xl font-bold font-display text-main mb-1">No encontramos ese dulce</h3>
      <p class="text-sm text-subtle mb-4">Prueba buscando con otro nombre o selecciona otra categoría.</p>
      <button
        @click="searchQuery = ''; activeCategory = 'all'"
        class="px-4 py-2 bg-primary text-white rounded-xl text-sm font-medium hover:bg-primary-hover transition-colors cursor-pointer"
      >
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      <article
        ...
        :key="product.id"
        class="group glass-card rounded-3xl overflow-hidden border border-border-subtle transition-all duration-300 hover:shadow-xl hover:-translate-y-1 flex flex-col justify-between"
      >
        <div>
          <!-- Product Image Container -->
          <div class="relative h-64 overflow-hidden bg-primary-soft">
            <img
            ...
            <span
              ...
              :class="product.category === 'ponquesitos' ? 'bg-primary/90 text-white' : 'bg-rose-600/90 text-white'"
            >
          ...
          <div class="p-6 pb-2">
            <div class="flex items-start justify-between gap-2 mb-2">
              <h3 class="text-xl font-bold font-display text-main group-hover:text-primary transition-colors line-clamp-1">
                {{ product.name }}
              </h3>
            </div>
            ...
            <div v-if="product.show_price && product.price" class="mb-3">
              <span class="inline-flex items-center text-lg font-bold text-primary-dark bg-primary-light px-3 py-1 rounded-xl border border-primary-border">
                {{ product.price }}
              </span>
            </div>
            <div v-else-if="product.show_price === false" class="mb-3">
              <span class="inline-flex items-center text-xs font-semibold text-subtle bg-surface-warm px-2.5 py-1 rounded-lg">
                Precio a consultar
              </span>
            </div>
        ...
        <div class="p-6 pt-4 border-t border-border-subtle/60 mt-auto">
          <a
